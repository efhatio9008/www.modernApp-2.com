...
app.post('/api/login', (req, res) => {
  ...
  if (username === ADMIN_USERNAME && passwordMatches(password)) {
...
app.get('/api/download', requireLogin, (req, res) => {
  try {
    ...
    if (!stat.isDirectory()) return res.download(target);
    res.setHeader('Content-Type', 'application/zip');
    res.setHeader('Content-Disposition', `attachment; filename="${path.basename(target)}.zip"`);
    const archive = archiver('zip', { zlib: { level: 9 } });
    archive.on('error', (error) => res.status(500).send(error.message));
    archive.pipe(res);
