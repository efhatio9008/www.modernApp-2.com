...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function loadCurrentFolder() {
    ...
    try {
      ...
      items.forEach((item) => renderExplorerItem(item));
    } catch (error) {
  ...
  function renderExplorerItem(item) {
    ...
    itemDiv.addEventListener('dblclick', function () {
      ...
      if (item.isFolder) openFolder(itemPath);
      else downloadItem(itemPath);
    });
    ...
    departmentOutput.appendChild(itemDiv);
  }
  ...
  function downloadItem(itemPath) {
    window.location.href = `/api/download?department=${encodeURIComponent(currentDepartmentRoot)}&path=${encodeURIComponent(itemPath || '')}`;
  }
  ...
  async function uploadFileList(files, parentPath, useRelativePath) {
    try {
      ...
      await showMessage('Upload', 'Upload completed successfully.');
    } catch (error) {
  ...
  async function bulkDeleteSelectedItems() {
    ...
    try {
      ...
      resetSelectMode();
      await loadCurrentFolder();
      await showMessage('Delete Selected', `${result.successCount} item(s) moved to Restore Deleted.${result.failCount ? ` ${result.failCount} item(s) failed.` : ''}`);
    } catch (error) {
  ...
  async function deleteItem(itemPath, isFolder) {
    ...
    try {
      ...
      await loadCurrentFolder();
      await showMessage('Delete', `${itemType.charAt(0).toUpperCase() + itemType.slice(1)} moved to Restore Deleted.`);
    } catch (error) {
  ...
  async function bulkRestoreDeletedItems(recordIds) {
    try {
      ...
      await loadCurrentFolder();
      await showMessage('Restore', `${result.successCount} item(s) restored.${result.failCount ? ` ${result.failCount} item(s) failed or were missing.` : ''}`);
    } catch (error) {
  ...
  async function bulkDeleteForeverItems(recordIds) {
    try {
      ...
      await loadCurrentFolder();
      await showMessage('Delete Forever', `${result.successCount} item(s) deleted forever.${result.failCount ? ` ${result.failCount} item(s) failed.` : ''}`);
    } catch (error) {
